...
def get_soft_scores_and_true_labels(dataset, model):
    ...
    with torch.no_grad():  
        ...
        all_second_soft_scores = torch.empty_like(all_first_soft_scores).to(device)
        gt_labels = torch.empty_like(all_first_soft_scores).to(device)  #grount truth
        ...
        for _, (inputs, targets) in enumerate(dataloader):
            ...
            all_first_soft_scores = torch.cat((output[:, 0], all_first_soft_scores))
            all_second_soft_scores = torch.cat((output[:, 1], all_second_soft_scores))
            gt_labels = torch.cat((targets, gt_labels))
    return (all_first_soft_scores.to('cpu'), all_second_soft_scores.to('cpu'), gt_labels.to('cpu'))
