...
def main():
    ...
    fakes_real_size = fakes_dataset_train.real_images_size
    real_image_idx = random.choice(range(0, fakes_real_size))
    fake_image_idx = random.choice(range(fakes_real_size + 1, len(fakes_dataset_train)))

    images_samples = plt.figure()
    plt.subplot(2, 2, 1)
    plt.imshow(normalize(
        fakes_dataset_train[real_image_idx][0]).permute(1, 2, 0))    
    plt.title('fakes dataset real image')
    plt.subplot(2, 2, 2)
    plt.imshow(normalize(
        fakes_dataset_train[fake_image_idx][0]).permute(1, 2, 0))    
    plt.title('fakes dataset fake image')

    # create synthetic faces dataset
    synthetic_dataset_train = load_dataset('synthetic_dataset', 'train')
    syntetihc_real_size = synthetic_dataset_train.real_images_size
    real_image_idx = random.choice(range(0, syntetihc_real_size))
    fake_image_idx = random.choice(range(syntetihc_real_size + 1, len(synthetic_dataset_train)))
