...
def compute_gradient_saliency_maps(samples: torch.tensor,
    ...
    target = torch.ones_like(true_labels)
    #(2) Compute a forward pass for the samples in the model.
    output = model(samples)
    output_val , _ = output.max(dim = 1)

    output_val.backward(gradient = target)       

    # (3) Gather only the scores which corresponds to the true labels of the samples.
    #    
    # scores_true_labels = output.gather(1, true_labels.view(-1, 1)).squeeze()
    # (4) Compute a backward pass on these scores.
    # scores_true_labels.backward(gradient=target , inputs = samples)
